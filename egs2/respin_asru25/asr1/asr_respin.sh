...
data_folder=data        # Directory to save data.
...
if [ "${lang}" != noinfo ]; then
    token_listdir=${data_folder}/${lang}${tag:+_$tag}_token_list
else
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        python local/score_lang_id.py --ref_text "${data_folder}/${valid_set}/text" \
            --hyp_text "${_dir}/text" \
            --utt2dial "${data_folder}/${valid_set}/utt2dial" \
            --out "${_dir}/result_lid.txt" || exit 1
    done
